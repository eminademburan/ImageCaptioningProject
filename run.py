...
def load_image(image_path):
    img = tf.io.read_file(IMAGE_DIRECTORY + image_path)
...
#32, 3, 299, 299
print("Recalculate image features ? (y or n):")
if input() == "y":
...
with open('captions.txt', 'r') as f:
    for line in f.readlines()[1:]: 
        ...
        image_captions.append('<start> ' + line[1] + ' <end>')

# Find the maximum length of any caption in our dataset
def calc_max_length(tensor):
    return max(len(t) for t in tensor)

# Choose the top 2500 words from the vocabulary
top_k = 2500
...
training_split = 0.8
slice_index = int(len(img_keys) * training_split)
...
def map_func(img_name, cap):
  img_tensor = np.load("./features/" + img_name.decode('utf-8') + '.npy')
...
def plot_attention(image, result, attention_plot):
    temp_image = np.array(Image.open(IMAGE_DIRECTORY + image))
